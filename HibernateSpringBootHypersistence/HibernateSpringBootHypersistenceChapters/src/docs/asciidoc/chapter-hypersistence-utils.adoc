...
Hibernate generates the SQL statements on your behalf, which is very convenient right up until
something runs slower than you expected, or a query you never intended to trigger shows up in the logs.
When that happens, there are three different questions you may need to answer:

* What is the SQL that a given JPQL or Criteria API query will be translated to?
* Which of your service methods fired a particular statement?
* What SQL queries does your application actually run in production?
...
where you want to make sure that a JPQL query compiles to the join or the predicate you expected:
...
`QueryStackTraceLogger` answers which of your methods fired a given query.
You provide it with your application base package, and whenever Hibernate executes a statement,
it will log the application stack frame responsible for triggering that particular query.
This is the fastest way to track down the origin of a query you did not expect,
because the log line points straight at the method in your own code,
rather than at some deep Hibernate internal frame that tells you nothing about your application.

To configure the `QueryStackTraceLogger`, you have to construct the instance and register it with Hibernate via the `statement_inspector` setting.
...
When you run it, the logger prints the SQL statement together with the stack trace that produced it.
Because the trace is walked only up to the first frame that belongs to your base package,
you see the Hibernate frames that generated the query,
and then the trace stops precisely at the method in your own code that triggered this stacktrace:
...
As you can see, in spite of the `@Retry` AOP Aspect, the `EntityManager`,
...
This is precisely what you want when a slow or unexpected statement shows up in production
and you need to find the code path that triggered it.

==== Recording queries as JFR events with `JfrQueryLogger`

`JfrQueryLogger` answers what your application actually ran,
...
you can open the recording and see the queries on the same timeline as the rest of the JVM activity.

You register it exactly like the previous tool, through a `HibernatePropertiesCustomizer` bean,
except that this time the instance you supply is a `new JfrQueryLogger()`.
Since a JFR event is just data, you capture the queries the same way you would capture any other Flight Recorder event,
such as garbage-collection pauses or virtual-thread pinning:
you start a `Recording` that enables the query event, run the code under test, stop the recording, and read the events back.
...
In a real application you would not read the recording back from a test,
...
and this is where you have to be careful, because Hibernate accepts only a single `StatementInspector` per `SessionFactory`.
So, if you register one of them directly, there is no room left for the other.
For this reason, when you want to run several inspectors at once,
you are better off wrapping them in a small composite that implements `StatementInspector` itself
...
as you can see in the field initializer above.
...
and to make your application faster, more robust, and easier to maintain.
Its features can be categorized into three groups.

The first group is about identifiers, which are the primary keys you put on your entities.
The `@Tsid` generator gives you identifiers that are sortable, batch-friendly, and non-guessable, all within 64 bits,
while `BatchSequenceGenerator` speeds up bulk imports without turning your database sequence into something that only Hibernate
knows how to use efficiently.

The second group is about Hibernate types, which allow you to get the best out of database-specific column types.
There is a great variety of types, such as `JsonType`, multiple array types, and types for ranges, intervals, `hstore`, and `citext`,
as well as the `YearMonthIntegerType` and `YearMonthDateType` that let you store a `YearMonth` using standard database column types.
However, Hibernate 7 has since caught up on plain JSON, simple arrays, `interval`, and `inet`,
so you are better off checking what Hibernate ORM already maps natively before reaching for the alternatives provided by this library.
...
The `@Retry` annotation retries only the failures that are transient, based on the exception list you provide,
...
Finally, `SQLExtractor`, `QueryStackTraceLogger`, and `JfrQueryLogger` tell you what SQL a query will be translated to,
which of your methods fired a given statement, and what SQL your application actually ran.
