...
or a new `BatchSequenceGenerator` that allows us to fetch multiple sequence values in a single roundtrip from a database sequence that has an increment step of just 1.
...
If we prefer the native mapping, we are better off overriding the `equals` and `hashCode` method of the POJO class wrapping the JSON content.
...
we would see that both execute a single `SELECT` statement, and both return the same five posts.
...
and then the trace stops precisely at the method in our own code that triggered this stacktrace:
...
the last frame names the exact method and line, `PostService.incrementLikes`, that started this stacktrace.
...
so reading them back gives us the three SQL statements the method issued,
...
while `BatchSequenceGenerator` speeds up bulk imports without turning our database sequence into something that only Hibernate
