    ...
    public ApplicationRunner init(ForumService forumService) {
        return args -> {
            Post post = new Post("Mapping polymorphic JSON with jsonb")
                ...
                .addBlock(new CodeBlock("java", "@Type(JsonBinaryType.class) List<PostContentBlock> body;"))
            ...
            var withJava = forumService.findPostsContaining("[{\"type\":\"code\",\"language\":\"java\"}]");
            LOGGER.info("Posts containing a Java code block (jsonb @>): {}", withJava.size());
        };
    }
}
